...
Building from source requires Make and a C compiler, *e.g.* GNU or LLVM. AALV targets the C99 standard and uses the standard and curses libraries. These should be available on Unix-like platforms with installed development tools. It also uses a modified version of [https://github.com/antirez/linenoise](https://github.com/antirez/linenoise) for line editing provided as a Git submodule.
...
## Motions
AALV attempts to faithfully mimic Vim-style motions. As AALV has an alignment-centric interface, there are additional commands to support left/right paging and manipulating the sizes of panes. Here's a short list of key differences and additions:
  ...
  - `^W`: change active pane with arrow key
...
## Commands
A small number of Ex ("colon") commands have been implemented. These include but aren't limited to:
  - `q`: quit
  - `n / p`: next/previous file
  - `set <name> [<value>]`: set option with value; options include `tick_offset`, `tick_spacing`, and `scheme`
  - `type <seq_type> [<id_pattern>]`: set type of sequence

See [cmd.c](src/cmd.c) for more details.
